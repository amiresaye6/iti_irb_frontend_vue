<template>
  <div class="space-y-6 p-4 md:p-6">
    ...
    <!-- Search Toolbar -->
    <SearchBar v-model="searchQuery" title="البحث والتصفية" search-label="البحث السريع"
      placeholder="ابحث برقم الملف أو عنوان البحث..." input-id="historySearchInput" @reset="handleReset">
    ...
    <DataTable :data="filteredData" :columns="columns" :loading="reviewStore.loading" :current-sort-column="sortCol"
      :current-sort-direction="sortDir" @sort="handleSort" class="mt-4">
      <template #cell(serial_number)="{ item }">
        <span
          class="text-neutral-content font-extrabold px-3 py-1.5 rounded-md text-sm whitespace-nowrap shadow-sm border border-neutral-content/20"
      ...
      <template #cell(assignment_status)="{ item }">
        <span v-if="item.assignment_status === 'awaiting_acceptance'"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#856404] bg-[#fff3cd] border border-[#ffeeba]">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        ...
        <span v-else-if="item.assignment_status === 'accepted'"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#155724] bg-[#d4edda] border border-[#c3e6cb]">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
        ...
        <span v-else-if="item.assignment_status === 'refused'"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#721c24] bg-[#f8d7da] border border-[#f5c6cb]">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
      ...
      <template #cell(decision)="{ item }">
        <template v-if="item.assignment_status === 'accepted'">
          <span v-if="item.decision === 'pending'"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#92400e] bg-[#fef3c7] border border-[#fde68a]">
            قيد الانتظار
          </span>
          <span v-else-if="item.decision === 'needs_modification'"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#92400e] bg-[#fff7ed] border border-[#fed7aa]">
            طلب تعديل
          </span>
          <span v-else-if="item.decision === 'approved'"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#166534] bg-[#dcfce7] border border-[#bbf7d0]">
            موافقة
          </span>
          <span v-else-if="item.decision === 'rejected'"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-bold whitespace-nowrap text-[#991b1b] bg-[#fee2e2] border border-[#fecaca]">
            مرفوض
          </span>
        </template>
        <span v-else-if="item.assignment_status === 'refused'"
          class="text-sm text-base-content/60 italic border-r-2 border-error pr-2 block w-max">
      ...
      <template #cell(assigned_at)="{ item }">
        <div class="flex flex-col gap-1">
          <span class="font-bold text-base-content text-[0.92rem]">{{ new
            Date(item.assigned_at).toLocaleDateString('ar-SA') }}</span>
      ...
      <template #row-actions="{ item }">
        <router-link :to="{ name: 'ReviewForm', params: { id: item.application_id } }"
