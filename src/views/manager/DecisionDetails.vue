        ...
        <div v-if="!application?.needs_modification" class="flex items-center justify-between bg-base-100 p-4 sm:p-6 rounded-2xl shadow-sm border border-base-200/60">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-base-content/60 mt-1">يرجى مراجعة تفاصيل البحث والملفات المرفقة بعناية قبل اتخاذ القرار النهائي.</p>
                </div>
            </div>
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8 w-full font-sans">
        ...
        <div class="flex items-center justify-between bg-base-100 p-4 sm:p-6 rounded-2xl shadow-sm border border-base-200/60">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-base-content">تفاصيل ومراجعة البحث</h1>
                    <p class="text-sm text-base-content/60 mt-1">يرجى مراجعة تفاصيل البحث والملفات المرفقة بعناية قبل اتخاذ القرار النهائي.</p>
                </div>
            </div>
            <button @click="router.push('/manager/dashboard')" class="btn btn-outline hover:btn-primary border-base-300 gap-2 px-6">
                العودة للوحة التحكم
            </button>
        </div>
        ...
        <div v-else-if="application" class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200/60 overflow-hidden">
                    <div class="px-6 py-5 border-b border-base-200/60 flex flex-wrap gap-4 items-center justify-between bg-base-50/50">
                        <h2 class="text-lg font-bold text-base-content">البيانات الأساسية</h2>
                        <div class="badge badge-primary px-4 py-3 text-sm font-medium shadow-sm">
                            {{ getStageName(application.current_stage) }}
                        </div>
                    </div>
                ...
                <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200/60 overflow-hidden">
                    ...
                    <div class="p-0">
                        <div v-if="docs.length > 0" class="divide-y divide-base-200/60">
                            <div v-for="doc in docs" :key="doc.id" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 hover:bg-base-200/20 transition-colors gap-4 group">
                                <div class="flex items-center gap-4">
                                ...
                                <a :href="`${Docs_BASE_URL}storage/${doc.file_path}`" target="_blank" class="btn btn-primary btn-sm px-8 h-10 font-bold w-full sm:w-auto shadow-sm">
                                    عرض المستند
                                </a>
                            </div>
                        </div>
                        <div v-else class="flex flex-col items-center justify-center py-16 text-center">
                            <p class="text-base-content/50 text-sm">لم يتم إرفاق أي ملفات خاصة بهذا البحث.</p>
                        </div>
                    </div>
                </div>
            </div>
...
import { useRoute, useRouter } from 'vue-router'
import { applicationServices } from '@/services/applicationServices.js'
import managerService from '@/services/managerService'; // تأكدي من استيراد سيرفيس المانجر للـ processDecision
...
// دالة معالجة الأزرار وإرسال الـ Actions للباكيند
const handleDecision = async (status) => {
    // التحقق من وجود ملاحظات للرفض والتعديل
    ...
    try {
        ...
        const appId = route.params.id;
        // مناداة الـ API المخصصة لمعالجة القرار
        const response = await managerService.processDecision(appId, payload);
        
        if (response && response.status === 'success') {
            alert(response.message);
            
            // التوجيه الذكي بناءً على رد الباكيند المكتوب فوق
            if (response.action === 'redirect_payment') {
                // التوجيه لصفحة تحديد الرسوم المالية
                router.push(`/admin/payments/set-fee/${appId}`);
            } else {
                // في حالة الرفض أو التعديل نرجعه للـ dashboard
                router.push('/manager/dashboard');
            }
        }
    } catch (err) {
        console.error('Error submitting decision:', err);
        alert('حدث خطأ أثناء إرسال القرار، يرجى المحاولة مرة أخرى.');
    } finally {
