...
const isManagerView = ref(route.path.includes('manager') || route.path.includes('admin'));
...
const handleDecision = async (status) => {
    ...
    try {
        ...
        const appId = route.params.id;
        const response = await managerService.processDecision(appId, payload);
        
        const statusResult = response?.status || response?.data?.status;
        const messageResult = response?.message || response?.data?.message;
        const actionResult = response?.action || response?.data?.action;

        if (statusResult === 'success') {
            alert(messageResult || 'تم تسجيل قرارك بنجاح');
            
            if (actionResult === 'redirect_payment') {
                router.push(`/admin/payments/set-fee/${appId}`);
            } else {
                router.push('/manager/dashboard');
            }
        } else {
            alert(messageResult || 'حدث خطأ غير متوقع');
        }
    } catch (err) {
        console.error('Error submitting decision:', err);
        const errorMsg = err.response?.data?.message || 'حدث خطأ أثناء إرسال القرار، يرجى المحاولة مرة أخرى.';
        alert(errorMsg);
    } finally {
...
<template>
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8 w-full font-sans">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-base-content">
                        {{ isManagerView ? 'تفاصيل ومراجعة البحث (لوحة الإدارة)' : 'تفاصيل البحث' }}
                    </h1>
                    <p class="text-sm text-base-content/60 mt-1">
                        {{ isManagerView ? 'يرجى مراجعة تفاصيل البحث والملفات المرفقة بعناية قبل اتخاذ القرار النهائي.' : 'عرض تفاصيل وحالة طلبك البحثي المستندات المرفقة.' }}
                    </p>
                </div>
            </div>
            
            <button 
                @click="router.push(isManagerView ? '/manager/dashboard' : '/student/dashboard')" 
                class="btn btn-outline hover:btn-primary border-base-300 gap-2 px-6"
            >
                ...
                العودة للأبحاث الأخرى الجاهزة للأعتماد 
            </button>
        </div>
        ...
        <div v-else-if="application" class="grid grid-cols-1" :class="{'lg:grid-cols-3 gap-8 items-start': isManagerView}">
            
            <div :class="{'lg:col-span-2 space-y-8': isManagerView, 'space-y-8': !isManagerView}">
                ...
                <div class="bg-base-100 rounded-2xl shadow-sm border border-base-200/60 overflow-hidden">
                    <div class="px-6 py-5 border-b border-base-200/60 flex flex-wrap gap-4 items-center justify-between bg-base-50/50">
                        <h2 class="text-lg font-bold text-base-content flex items-center gap-2">البيانات الأساسية</h2>
                ...
                <div v-if="isManagerView" class="p-4 bg-primary/5 rounded-2xl border border-primary/10 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">🔍</span>
                        <p class="text-sm font-medium text-base-content/80">هل انتهيت من فحص كافة المرفقات؟ يمكنك الآن اتخاذ القرار من نموذج التحكم.</p>
                    </div>
                    <a href="#decision-card" class="btn btn-sm btn-primary px-5">الانتقال للقرار بالأعلى</a>
                </div>

            </div>

            <div v-if="isManagerView" id="decision-card" class="lg:col-span-1 sticky top-6">
                <div class="card bg-base-100 shadow-xl border border-base-200">
                    <div class="card-body">
                        <h3 class="card-title text-error border-b pb-2 mb-4 flex items-center gap-2">
                            <span>⚖️</span> اتخاذ القرار النهائي
                        </h3>
                        
                        <div class="form-control w-full mb-4">
                            <label class="label">
                                <span class="label-text font-semibold">ملاحظات وتوجيهات المدير:</span>
                            </label>
                            <textarea 
                                v-model="notes" 
                                class="textarea textarea-bordered h-28 w-full focus:textarea-primary" 
                                placeholder="اكتب هنا أسباب الرفض، التعديلات المطلوبة، أو ملحوظات القبول..."
                            ></textarea>
                        </div>
